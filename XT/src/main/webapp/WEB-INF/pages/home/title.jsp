...
<head>
    ...
    <link rel="stylesheet" rev="stylesheet" type="text/css" href="${ctx}/staticfile/skin/default/css/title.css" media="all"/>
...
<script language="javascript">
	...
	function CustomTitle(){
		...
		Me.onmouseover = function(){document.getElementById('memo').style.background='url(${ctx}/staticfile/skin/default/images/title/memo2.gif) no-repeat'};
		Me.onmouseout = function(){document.getElementById('memo').style.background='url(${ctx}/staticfile/skin/default/images/title/memo.gif) no-repeat'};
		
		Loin.onmouseover = function(){document.getElementById('small_login').style.background='url(${ctx}/staticfile/skin/default/images/title/small_login2.gif) no-repeat'};
		Loin.onmouseout = function(){document.getElementById('small_login').style.background='url(${ctx}/staticfile/skin/default/images/title/small_login.gif) no-repeat'};
		
		Lout.onmouseover = function(){document.getElementById('small_login_out').style.background='url(${ctx}/staticfile/skin/default/images/title/login_out2.gif) no-repeat left -55px;'};
		Lout.onmouseout = function(){document.getElementById('small_login_out').style.background='url(${ctx}/staticfile/skin/default/images/title/login_out.gif) no-repeat'};
	}
	...
	function linkHighlightMenu(obj){
		...
		for(var i=0;i<links.length;i++){
			if(links[i].id.indexOf('topmenu')!=-1){
				links[i].style.background = 'url("${ctx}/staticfile/skin/default/images/title/li_dot.gif1") no-repeat right 8px';
				links[i].style.color = '';
...
<div class="PositionFrame_black" id="PositionFrame"></div>
	<div id="userInfo" style="z-index:999;" onclick="HideLoginDiv()" title="点击关闭">
		...
		您好：<strong>${user.userInfo.name}</strong>&nbsp;&nbsp;|
		您所属单位：<strong>
			<c:if test="${!empty user.dept}">
			${user.dept.deptName}
			</c:if>
		</strong>&nbsp;&nbsp;
	...
	<div class="top_logo">
	    <div class="navMenu"  style="float:left;text-align:left;">
	    		...
	    		<div style="height:29px;">
		    		<span id="leftKey"  onmouseover="periodOffset(this, 'left')"><img src="${ctx}/staticfile/skin/default/images/title/left_arrow.png"/></span>
...
<div id="prompt_div"><img src="${ctx}/staticfile/skin/default/images/title/prompt.png"/><span style="position:absolute;top:2px;left:35px;z-index: 99999;width:100%;color:#FFFFFF;text-align: left; ">鼠标指向箭头位置<br/>可显示更多菜单项</span></div>
